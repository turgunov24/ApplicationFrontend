...
type IService = IIndexResponse['result'][number];
...
export default function Page() {
  ...
  const [idForDeleteUser, setIdForDeleteUser] = useState<IService['id'] | null>(null);
      ...
      columnHelper.accessor('id', {
        ...
        cell: (info) => (
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <RenderElementByPermission permissions={[referencesServicesPermissions.update]}>
              <Tooltip title="Quick edit" placement="top" arrow>
                <IconButton
                  color="info"
                  onClick={() => {
                    setQueryStates({ serviceId: info.getValue(), formOpen: true });
                  }}
                >
            ...
            <RenderElementByPermission permissions={[referencesServicesPermissions.delete]}>
  ...
  const {
    ...
    queryKey: [REFERENCES_SERVICES_BASE_QUERY_KEY, 'getCountsByStatus'],
    queryFn: async () => {
      try {
        const response = await referencesServicesService.helpers.getCountsByStatus();
        return response;
      } catch (error: unknown) {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        const response = await referencesServicesService.index({
          status,
          search,
  ...
  const { mutate: deleteUser } = useMutation({
    mutationKey: [REFERENCES_SERVICES_BASE_QUERY_KEY, 'delete'],
    mutationFn: async (id: IService['id']) => {
      try {
        const response = await referencesServicesService.form.delete(id);
        return response;
      } catch (error: unknown) {
        console.log('error', error);
        return false;
    ...
    <>
      ...
      <DashboardContent>
        <CustomBreadcrumbs
          ...
          action={
            <RenderElementByPermission permissions={[referencesServicesPermissions.create]}>
              <Button
                variant="contained"
