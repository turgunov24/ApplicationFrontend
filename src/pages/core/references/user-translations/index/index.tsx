...
type IUserTranslation = IIndexResponse['result'][number];

const metadata = { title: `User Translations - ${CONFIG.appName}` };
const fallBackData: any[] = [];

export default function Page() {
  const queryClient = useQueryClient();

  const [idForDeleteUser, setIdForDeleteUser] = useState<IUserTranslation['id'] | null>(null);
  const [sorting, setSorting] = useState<SortingState>([]);
  const [rowSelection, setRowSelection] = useState({});
  const [dense, setDense] = useState(false);

  const [{ search, formOpen, ...pagination }, setQueryStates] = useQueryStates(
    {
  ...
  const columnHelper = createColumnHelper<IUserTranslation>();
      ...
      columnHelper.accessor('id', {
        ...
        cell: (info) => (
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <RenderElementByPermission permissions={[referencesUserTranslationsPermissions.update]}>
              <Tooltip title="Quick edit" placement="top" arrow>
                <IconButton
                  color="info"
                  onClick={() => {
                    setQueryStates({ userTranslationId: info.getValue(), formOpen: true });
                  }}
                >
            ...
            <RenderElementByPermission permissions={[referencesUserTranslationsPermissions.delete]}>
  ...
  const {
    ...
    queryKey: [
      REFERENCES_USER_TRANSLATIONS_BASE_QUERY_KEY,
      'index',
    ...
    queryFn: async () => {
      try {
        const response = await referencesUserTranslationsService.index({
          search,
  ...
  const { mutate: deleteUser } = useMutation({
    mutationKey: [REFERENCES_USER_TRANSLATIONS_BASE_QUERY_KEY, 'delete'],
    mutationFn: async (id: IUserTranslation['id']) => {
      try {
        const response = await referencesUserTranslationsService.form.delete(id);
        return response;
      } catch (error: unknown) {
  ...
  return (
    <>
      ...
      <DashboardContent>
        <CustomBreadcrumbs
          heading="List"
          links={[{ name: 'Dashboard', href: paths.dashboard.root }, { name: 'User Translations' }]}
          action={
            <RenderElementByPermission permissions={[referencesUserTranslationsPermissions.create]}>
              <Button
                variant="contained"
