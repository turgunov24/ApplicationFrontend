...
type ITranslation = IIndexResponse['result'][number];

const metadata = { title: `Translations - ${CONFIG.appName}` };
...
export default function Page() {
  ...
  const [idForDeleteUser, setIdForDeleteUser] = useState<ITranslation['id'] | null>(null);
  ...
  const columnHelper = createColumnHelper<ITranslation>();
      ...
      columnHelper.accessor('id', {
        ...
        cell: (info) => (
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <RenderElementByPermission permissions={[referencesTranslationsPermissions.update]}>
              <Tooltip title="Quick edit" placement="top" arrow>
                <IconButton
                  color="info"
                  onClick={() => {
                    setQueryStates({ translationId: info.getValue(), formOpen: true });
                  }}
                >
            ...
            <RenderElementByPermission permissions={[referencesTranslationsPermissions.delete]}>
  ...
  const {
    ...
    queryKey: [REFERENCES_TRANSLATIONS_BASE_QUERY_KEY, 'getCountsByStatus'],
    queryFn: async () => {
      try {
        const response = await referencesTranslationsService.helpers.getCountsByStatus();
        return response;
      } catch (error: unknown) {
  ...
  const {
    ...
    queryKey: [
      REFERENCES_TRANSLATIONS_BASE_QUERY_KEY,
      'index',
    ...
    queryFn: async () => {
      try {
        const response = await referencesTranslationsService.index({
          status,
          search,
  ...
  const { mutate: deleteUser } = useMutation({
    mutationKey: [REFERENCES_TRANSLATIONS_BASE_QUERY_KEY, 'delete'],
    mutationFn: async (id: ITranslation['id']) => {
      try {
        const response = await referencesTranslationsService.form.delete(id);
        return response;
      } catch (error: unknown) {
        console.log('error', error);
        return false;
  ...
  return (
    <>
      ...
      <DashboardContent>
        <CustomBreadcrumbs
          heading={t('demo.description')}
          links={[{ name: 'Dashboard', href: paths.dashboard.root }, { name: 'Translations' }]}
          action={
            <RenderElementByPermission permissions={[referencesTranslationsPermissions.create]}>
              <Button
                variant="contained"
