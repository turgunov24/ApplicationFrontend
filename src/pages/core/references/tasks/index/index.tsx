...
type ITask = IIndexResponse['result'][number];

const metadata = { title: `Tasks - ${CONFIG.appName}` };
const fallBackData: any[] = [];

export default function Page() {
  const queryClient = useQueryClient();

  const [idForDeleteUser, setIdForDeleteUser] = useState<ITask['id'] | null>(null);
        ...
        cell: (info) => (
          <Label
            ...
            color={
              ...
              (info.getValue() === 'finished' && 'info') ||
              (info.getValue() === 'cancelled' && 'error') ||
              (info.getValue() === 'deleted' && 'error') ||
              (info.getValue() === 'archived' && 'warning') ||
              'default'
            }
          >
            {info.getValue()}
      ...
      columnHelper.accessor('id', {
        ...
        cell: (info) => (
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <RenderElementByPermission permissions={[referencesTasksPermissions.update]}>
              <Tooltip title="Quick edit" placement="top" arrow>
                <IconButton
                  color="info"
                  onClick={() => {
                    setQueryStates({ taskId: info.getValue(), formOpen: true });
                  }}
                >
            ...
            <RenderElementByPermission permissions={[referencesTasksPermissions.delete]}>
  ...
  const {
    ...
    data: countsByStatus = {
      ...
      [StatusesEnum.send]: 0,
      [StatusesEnum.cancelled]: 0,
      [StatusesEnum.archived]: 0,
      [StatusesEnum.deleted]: 0,
    },
  } = useQuery({
    queryKey: [REFERENCES_TASKS_BASE_QUERY_KEY, 'getCountsByStatus'],
    queryFn: async () => {
      try {
        const response = await referencesTasksService.helpers.getCountsByStatus();
        return response;
      } catch (error: unknown) {
  ...
  const {
    ...
    queryKey: [
      REFERENCES_TASKS_BASE_QUERY_KEY,
      'index',
    ...
    queryFn: async () => {
      try {
        const response = await referencesTasksService.index({
          status,
          search,
  ...
  const { mutate: deleteUser } = useMutation({
    mutationKey: [REFERENCES_TASKS_BASE_QUERY_KEY, 'delete'],
    mutationFn: async (id: ITask['id']) => {
      try {
        const response = await referencesTasksService.form.delete(id);
        return response;
      } catch (error: unknown) {
        console.log('error', error);
        return false;
  ...
  return (
    <>
      ...
      <DashboardContent>
        <CustomBreadcrumbs
          heading="List"
          links={[{ name: 'Dashboard', href: paths.dashboard.root }, { name: 'Tasks' }]}
          action={
            <RenderElementByPermission permissions={[referencesTasksPermissions.create]}>
              <Button
                variant="contained"
