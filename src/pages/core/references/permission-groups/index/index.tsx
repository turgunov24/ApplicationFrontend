...
type IPermissionGroup = IIndexResponse['result'][number];

const metadata = { title: `Permission Groups - ${CONFIG.appName}` };
const fallBackData: any[] = [];

export default function Page() {
  const queryClient = useQueryClient();

  const [idForDeleteUser, setIdForDeleteUser] = useState<IPermissionGroup['id'] | null>(null);
  ...
  const columnHelper = createColumnHelper<IPermissionGroup>();
      ...
      columnHelper.accessor('id', {
        ...
        cell: (info) => (
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <RenderElementByPermission permissions={[referencesPermissionGroupsPermissions.update]}>
              <Tooltip title="Quick edit" placement="top" arrow>
                <IconButton
                  color="info"
                  onClick={() => {
                    setQueryStates({ permissionGroupId: info.getValue(), formOpen: true });
                  }}
                >
            ...
            <RenderElementByPermission permissions={[referencesPermissionGroupsPermissions.delete]}>
  ...
  const {
    ...
    queryKey: [REFERENCES_PERMISSION_GROUPS_BASE_QUERY_KEY, 'getCountsByStatus'],
    queryFn: async () => {
      try {
        const response = await referencesPermissionGroupsService.helpers.getCountsByStatus();
        return response;
      } catch (error: unknown) {
  ...
  const {
    ...
    queryKey: [
      REFERENCES_PERMISSION_GROUPS_BASE_QUERY_KEY,
      'index',
    ...
    queryFn: async () => {
      try {
        const response = await referencesPermissionGroupsService.index({
          status,
          search,
  ...
  const { mutate: deleteUser } = useMutation({
    mutationKey: [REFERENCES_PERMISSION_GROUPS_BASE_QUERY_KEY, 'delete'],
    mutationFn: async (id: IPermissionGroup['id']) => {
      try {
        const response = await referencesPermissionGroupsService.form.delete(id);
        return response;
      } catch (error: unknown) {
        console.log('error', error);
        return false;
  ...
  return (
    <>
      ...
      <DashboardContent>
        <CustomBreadcrumbs
          heading="List"
          links={[{ name: 'Dashboard', href: paths.dashboard.root }, { name: 'Permission Groups' }]}
          action={
            <RenderElementByPermission permissions={[referencesPermissionGroupsPermissions.create]}>
              <Button
                variant="contained"
