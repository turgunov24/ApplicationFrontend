...
type ILegalForm = IIndexResponse['result'][number];

const metadata = { title: `Legal Forms - ${CONFIG.appName}` };
const fallBackData: any[] = [];

export default function Page() {
  const queryClient = useQueryClient();

  const [idForDeleteUser, setIdForDeleteUser] = useState<ILegalForm['id'] | null>(null);
  ...
  const columnHelper = createColumnHelper<ILegalForm>();
      ...
      columnHelper.accessor('id', {
        ...
        cell: (info) => (
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <RenderElementByPermission permissions={[referencesLegalFormsPermissions.update]}>
              <Tooltip title="Quick edit" placement="top" arrow>
                <IconButton
                  color="info"
                  onClick={() => {
                    setQueryStates({ legalFormId: info.getValue(), formOpen: true });
                  }}
                >
            ...
            <RenderElementByPermission permissions={[referencesLegalFormsPermissions.delete]}>
  ...
  const {
    ...
    queryKey: [REFERENCES_LEGAL_FORMS_BASE_QUERY_KEY, 'getCountsByStatus'],
    queryFn: async () => {
      try {
        const response = await referencesLegalFormsService.helpers.getCountsByStatus();
        return response;
      } catch (error: unknown) {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        const response = await referencesLegalFormsService.index({
          status,
          search,
  ...
  const { mutate: deleteUser } = useMutation({
    mutationKey: [REFERENCES_LEGAL_FORMS_BASE_QUERY_KEY, 'delete'],
    mutationFn: async (id: ILegalForm['id']) => {
      try {
        const response = await referencesLegalFormsService.form.delete(id);
        return response;
      } catch (error: unknown) {
        console.log('error', error);
        return false;
  ...
  return (
    <>
      ...
      <DashboardContent>
        <CustomBreadcrumbs
          heading="List"
          links={[{ name: 'Dashboard', href: paths.dashboard.root }, { name: 'Legal Forms' }]}
          action={
            <RenderElementByPermission permissions={[referencesLegalFormsPermissions.create]}>
              <Button
                variant="contained"
