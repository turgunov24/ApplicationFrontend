...
export default function Page() {
  ...
  const {
    ...
    queryKey: [REFERENCES_COUNTRIES_BASE_QUERY_KEY, 'getCountsByStatus'],
    queryFn: async () => {
      try {
        const response = await referencesCountriesService.helpers.getCountsByStatus();
        return response;
      } catch (error: unknown) {
  ...
  const {
    ...
    queryKey: [
      REFERENCES_COUNTRIES_BASE_QUERY_KEY,
      'index',
    ...
    queryFn: async () => {
      try {
        const response = await referencesCountriesService.index({
          status,
          search,
  ...
  const { mutate: deleteUser } = useMutation({
    mutationKey: [REFERENCES_COUNTRIES_BASE_QUERY_KEY, 'delete'],
    mutationFn: async (id: ICountry['id']) => {
      try {
        const response = await referencesCountriesService.form.delete(id);
        return response;
      } catch (error: unknown) {
        console.log('error', error);
        return false;
