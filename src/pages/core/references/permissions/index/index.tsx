...
import { referencesPermissionsPermissions } from '../helpers/permissions'
import { referencesPermissionsService, REFERENCES_PERMISSIONS_BASE_QUERY_KEY } from '../services'

// ----------------------------------------------------------------------

type IPermission = IIndexResponse['result'][number];

const metadata = { title: `Permissions - ${CONFIG.appName}` };
const fallBackData: any[] = [];

export default function Page() {
  const queryClient = useQueryClient();

  const [idForDeleteUser, setIdForDeleteUser] = useState<IPermission['id'] | null>(null);
      ...
      columnHelper.accessor('id', {
        ...
        cell: (info) => (
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <RenderElementByPermission permissions={[referencesPermissionsPermissions.update]}>
              <Tooltip title="Quick edit" placement="top" arrow>
                <IconButton
                  color="info"
                  onClick={() => {
                    setQueryStates({ formOpen: true, permissionId: info.getValue() });
                  }}
                >
            ...
            <RenderElementByPermission permissions={[referencesPermissionsPermissions.delete]}>
  ...
  const {
    ...
    queryKey: [REFERENCES_PERMISSIONS_BASE_QUERY_KEY, 'getCountsByStatus'],
    queryFn: async () => {
      try {
        const response = await referencesPermissionsService.helpers.getCountsByStatus();
        return response;
      } catch (error: unknown) {
  ...
  const {
    ...
    queryKey: [
      REFERENCES_PERMISSIONS_BASE_QUERY_KEY,
      'index',
    ...
    queryFn: async () => {
      try {
        const response = await referencesPermissionsService.index({
          status,
          search,
  ...
  const { mutate: deleteUser } = useMutation({
    mutationKey: [REFERENCES_PERMISSIONS_BASE_QUERY_KEY, 'delete'],
    mutationFn: async (id: IPermission['id']) => {
      try {
        const response = await referencesPermissionsService.form.delete(id);
        return response;
      } catch (error: unknown) {
        console.log('error', error);
        return false;
  ...
  return (
    <>
      ...
      <DashboardContent>
        <CustomBreadcrumbs
          heading="List"
          links={[{ name: 'Dashboard', href: paths.dashboard.root }, { name: 'Permissions' }]}
          action={
            <RenderElementByPermission permissions={[referencesPermissionsPermissions.create]}>
              <Button
                variant="contained"
