...
import {
  attachTemplateToTaskService,
  ATTACH_TEMPLATE_TO_TASK_BASE_QUERY_KEY,
} from '../services';
...
const metadata = { title: `Attach Template to Task - ${CONFIG.appName}` };
...
export default function Page() {
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('taskTemplate', {
        header: 'Template',
        cell: (info) => info.getValue()?.translationKey ?? '-',
      }),
      ...
      columnHelper.accessor('id', {
        ...
        cell: (info) => (
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <RenderElementByPermission permissions={[attachTemplateToTaskPermissions.update]}>
              <Tooltip title="Quick edit" placement="top" arrow>
                <IconButton
                  color="info"
                  onClick={() => {
                    setQueryStates({ attachTemplateToTaskId: info.getValue(), formOpen: true });
                  }}
                >
            ...
            <RenderElementByPermission permissions={[attachTemplateToTaskPermissions.delete]}>
  ...
  const {
    ...
    queryKey: [ATTACH_TEMPLATE_TO_TASK_BASE_QUERY_KEY, 'getCountsByStatus'],
    queryFn: async () => {
      try {
        const response = await attachTemplateToTaskService.helpers.getCountsByStatus();
        return response;
      } catch (error: unknown) {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        const response = await attachTemplateToTaskService.index({
          status,
          search,
  ...
  const { mutate: deleteRecord } = useMutation({
    mutationKey: [ATTACH_TEMPLATE_TO_TASK_BASE_QUERY_KEY, 'delete'],
    mutationFn: async (id: IRecord['id']) => {
      try {
        const response = await attachTemplateToTaskService.form.delete(id);
        return response;
      } catch (error: unknown) {
        console.log('error', error);
        return false;
  ...
  return (
    <>
      ...
      <DashboardContent>
        <CustomBreadcrumbs
          heading="List"
          links={[{ name: 'Dashboard', href: paths.dashboard.root }, { name: 'Attach Template to Task' }]}
          action={
            <RenderElementByPermission permissions={[attachTemplateToTaskPermissions.create]}>
              <Button
                variant="contained"
