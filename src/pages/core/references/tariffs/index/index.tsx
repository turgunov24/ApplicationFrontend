...
type ITariff = IIndexResponse['result'][number];

const metadata = { title: `Tariffs - ${CONFIG.appName}` };
...
export default function Page() {
  ...
  const {
    ...
    queryKey: [REFERENCES_TARIFFS_BASE_QUERY_KEY, 'getCountsByStatus'],
    queryFn: async () => {
      try {
        const response = await referencesTariffsService.helpers.getCountsByStatus();
        return response;
      } catch (error: unknown) {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        const response = await referencesTariffsService.index({
          status,
          search,
