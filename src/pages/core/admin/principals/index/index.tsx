...
import { principalsPermissions } from '../helpers/permissions';
import { principalsService, PRINCIPALS_BASE_QUERY_KEY } from '../services';

// ----------------------------------------------------------------------

type IPrincipal = IIndexResponse['result'][number];

const metadata = { title: `Principals - ${CONFIG.appName}` };
...
export default function Page() {
  ...
  const [idForDeletePrincipal, setIdForDeletePrincipal] = useState<IPrincipal['id'] | null>(null);
  ...
  const columnHelper = createColumnHelper<IPrincipal>();
      ...
      columnHelper.accessor('id', {
        ...
        cell: (info) => (
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <RenderElementByPermission permissions={[principalsPermissions.update]}>
            ...
            <RenderElementByPermission permissions={[principalsPermissions.delete]}>
              <IconButton color="error" onClick={() => setIdForDeletePrincipal(info.getValue())}>
  ...
  const {
    ...
    queryKey: [PRINCIPALS_BASE_QUERY_KEY, 'getCountsByStatus'],
    queryFn: async () => {
      try {
        const response = await principalsService.helpers.getCountsByStatus();
        return response;
      } catch (error: unknown) {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        const response = await principalsService.index({
          status,
          search,
  ...
  const { mutate: deletePrincipal } = useMutation({
    mutationKey: [PRINCIPALS_BASE_QUERY_KEY, 'delete'],
    mutationFn: async (id: IPrincipal['id']) => {
      try {
        const response = await principalsService.form.delete(id);
        return response;
      } catch (error: unknown) {
        console.log('error', error);
        return false;
  ...
  const renderConfirmDialog = () => (
    <ConfirmDialog
      open={!!idForDeletePrincipal}
      onClose={() => setIdForDeletePrincipal(null)}
      title="Delete"
      ...
      action={
        <Button
          ...
          color="error"
          onClick={() => {
            deletePrincipal(idForDeletePrincipal!);
            setIdForDeletePrincipal(null);
          }}
        >
          Delete
  ...
  return (
    <>
      ...
      <DashboardContent>
        <CustomBreadcrumbs
          heading="List"
          links={[{ name: 'Dashboard', href: paths.dashboard.root }, { name: 'Principals' }]}
          action={
                        ...
                        <Stack direction="row" alignItems="center" justifyContent="space-between">
                          ...
                          <Tooltip title="Delete">
                            <IconButton
                              color="primary"
                              onClick={() => {
                                // setIdForDeletePrincipal(table.getSelectedRowModel().rows[0].original.id);
                              }}
                            >
                              <Iconify icon="solar:trash-bin-trash-bold" />
                            </IconButton>
